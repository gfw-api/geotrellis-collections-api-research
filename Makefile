...
server:
	...
	target/scala-2.11/geotrellis_collections_api-assembly-1.2.1.jar"
...
download-tif:
	curl -o nsa_all_years.tif \
	http://s3.amazonaws.com/palm-risk-poc/data/glad/nsa_all_years.tif;
	hdfs dfs -mkdir /data;
	hdfs dfs -mkdir /data/glad;
	hdfs dfs -put nsa_all_years.tif /data/
...
ingest: ingest-assembly download-tif
	...
	target/scala-2.11/geotrellis_collections_api_ingest-assembly-1.2.1.jar"
